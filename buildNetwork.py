...
def makeAnalog(analog, memory):
    ...
    for item in analog:
        # for legacy reasons, first check if this item is from a sym file with the 'group' field. 
        if 'group' in item: 
            # create a group unit if you should ('group' is true (i.e., the prop specifies a group)). 
            if item['group'] == True:
                # MAYBE: make the group, and make any props if it has props... 
                newGroup = dataTypes.GroupUnit(item['name'], item['set'], item['analog'], False, new_analog)
                # add my semantics if I've got any. 
                
                # make my props if I've got any. 
                for prop in item['props']: 
                    memory = makeProposition(item, memory, new_analog)
            else: 
                # just make the proposition (with no group units). 
                memory = makeProposition(item, memory, new_analog)
        else:
            # this sym file is legacy (it has no 'group' field, so just create the propostion that isn't part of a group. 
            memory = makeProposition(item, memory, new_analog)
...
# code to make the elements in a proposition. 
def makeProposition(prop, memory, new_analog):
    ...
    for myRB in prop['RBs']:
        ...
        if newRB != 'non_exist':
            ...
            if add_new_pred and newPred != 'non_exist':
                ...
                for semantic in pred_sem_list:
                    ...
                    makeNewSem, newSem = checkMakeNewSem(semantic, memory)
                    ...
                    if makeNewSem:
                        ...
                        if (type(semantic) is list) and (len(semantic) > 5): # ekaterina
                            ...
                            newLink = dataTypes.Link(newPred, None, None, newSem, semantic[1])
                        ...
                        # check wheter semantic codes a dimension or not.
                            ...
                            newLink = dataTypes.Link(newPred, None, None, newSem, 1)
        ...
        # if the RB item is not higher-order, make the newobject. NOTE: the myRB object is the dict from the sym file, so this code will make the object either when the myRB dict specifies a pred and RB (and you've made a newPred above), or when the myRB dict specifies a single object unlinked to any other tokens. 
                ...
                for semantic in obj_sem_list:
                    ...
                    if makeNewSem:
                        ...
                        if (type(semantic) is list) and (len(semantic) > 4):
                            newSem = dataTypes.Semantic(semantic[0], semantic[2], semantic[3], semantic[4])
                            newLink = dataTypes.Link(newObject, None, None, newSem, semantic[1])
                        elif (type(semantic) is list) and (len(semantic) > 2):
                            newSem = dataTypes.Semantic(semantic[0], semantic[2], semantic[3])
                            newLink = dataTypes.Link(newObject, None, None, newSem, semantic[1])
                        elif type(semantic) is list:
                            ...
                            # default to a semantic with a weight of 1.
                            newSem = dataTypes.Semantic(semantic)
                            newLink = dataTypes.Link(newObject, None, None, newSem, 1)
                        # add newLink to newSem and newPred, and add newLink to currentLinks, and newSem to memory.semantics.
                        newSem.myPOs.append(newLink)
...
def checkMakeNewSem(semantic, memory):
    makeNewSem = True
    newSem = None
    # is the semantic in the list of current semantics?
    if type(semantic) is list:
        for oldsemantic in memory.semantics:
            if oldsemantic.name == semantic[0]:
                makeNewSem = False
        ...
        for oldsemantic in memory.semantics:
            if oldsemantic.name == semantic:
                makeNewSem = False
                newSem = oldsemantic # set newSem to oldsemantic; newSem is the semantic unit that's going to get updated. 
                break
    # done. 
    return makeNewSem, newSem
